...
import com.yahoo.oak.BasicChunk.State;
...
class InternalOakMap<K, V>  extends InternalOakBasics<K, V> {
    ...
    OakUnscopedBuffer get(K key) { //TODO what we do here?
        if (key == null) {
            throw new NullPointerException();
        }
    ...
    <T> T getKeyTransformation(K key, OakTransformer<T> transformer) { //TODO what we do here?
        if (key == null) {
            throw new NullPointerException();
        }
    ...
    V replace(K key, V value, OakTransformer<V> valueDeserializeTransformer) {
        ...
        for (int i = 0; i < MAX_RETRIES; i++) {
            try {
                ...
                Result result = 
                        valueOperator.exchange(c, ctx, value, valueDeserializeTransformer, getValueSerializer());
    ...
    Map.Entry<K, V> lowerEntry(K key) { //TODO what we do here?
    ...
    abstract class OrderedIter<T> extends  BasicIter<T> {
        ...
        protected BasicChunk<K, V> getNextChunk(BasicChunk<K, V> current) {
            ...
            if (current.state.get() == State.RELEASED) { //release the chunk now
                currentHashChunk.entryOrderedSet.releaseAllDeletedKeys();
            }
            if (!isDescending) {
        ...
        protected void advanceState() {
            ...
            orderedChunk.referenceCount.addAndGet(1); // making the ref count 1, so that keys are not released
